...
- `openwiki/development.md` — dev cycle, providers, token budget, pre-release
...
- Pre-release checklist in `openwiki/development.md`.
...
`CLAUDE.local.md` is auto-generated by headroom. Do not modify it. If it contradicts openwiki, CLAUDE.local.md is fresher.
`openwiki/` is regenerated by CI (`.github/workflows/openwiki-update.yml`, manual dispatch only). Regeneration can undo de-duplication edits — review its PR diff before merging.
