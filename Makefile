...
test: $(PLENARY)
	...
	@awk '{ gsub(/\033\[[0-9;]*m/, "") } /Success[[:space:]]*:/ { s+=$$2 } /Failed[[:space:]]*:/ { f+=$$2 } /Errors[[:space:]]*:/ { e+=$$2 } END { printf "TOTAL: %d passed, %d failed, %d errors\n", s, f, e; exit (f > 0 || e > 0 || s == 0) }' $(DEPS)/last-test.log || { echo "── FAILED — full log below ($(DEPS)/last-test.log) ──"; cat $(DEPS)/last-test.log; exit 1; }
